...
raw_path = f"./Test Images/GrayImages/Baboon.raw"
# raw_path = f"./LenaRGB_RGB_422.raw"
img = read_raw(raw_path,(512,512,1),dtype=np.uint8)
print(type(img))
print(np.shape(img))
img = cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
cv2.imwrite(f"Baboon.jpg",img)


for QF in [80,5, 10, 20, 50, 90]:
  img = read_raw(f"./LenaRGB_QF{QF}_decoded.raw",(512,512,3),dtype=np.uint8)
  print(type(img))
  print(np.shape(img))
  img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
  ...
  cv2.imwrite(f"LenaRGB_{QF}.jpg",img)